...
import { oauthRecovery } from "./recovery.js";

/** Preserve generated transports while allowing one identity-bound OAuth recovery. */
	...
	protected override async makeRequest(
		...
		try {
			try {
				...
				if (!(error instanceof APIError) || error.status !== 401 || options.signal?.aborted)
					throw error;
				// Streams cannot safely be replayed after the first request consumes them.
				if (
					options.body instanceof ReadableStream ||
					(options.body && typeof options.body === "object" && Symbol.asyncIterator in options.body)
				)
					throw error;

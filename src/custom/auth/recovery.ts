...
export const recoverableBearer = async (initial: string, store: CredentialStore, provider: AuthProvider): Promise<() => string> => {
  const identity = await store.read()
  if (!identity || identity.accessToken !== initial) throw new CredentialStorageError('invalid_credential')
  let token = initial
  const bearer = () => token
  recoveries.set(bearer, { recover: async (rejected) => {

...
import type { MachineAPI } from '../commands/machines.js'
...
export const connectMachine = async (api: MachineAPI, machineID: string): Promise<void> => {
  ...
  const trackChild = (value: ChildProcess | undefined): void => {
    child = value
  }
...
export const awaitSSHSession = async (
  ...
  for (let poll = 0; poll <= pollLimit; poll += 1) {
    ...
    switch (status) {
      ...
      case 'expired':
        throw new Error(
          'SSH session expired before it became ready; try again with a fresh session',
        )
      case 'closed':

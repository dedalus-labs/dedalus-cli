...
const formatDedalusError = (...args: Parameters<typeof formatError>) => {
  const result = formatError(...args)
  assert(result, 'Expected a structured auth error')
  return result
}
...
const bearerToken = (value: unknown): unknown => {
...
const resourceProgram = (onAction: (command: Command) => void) =>
  new Command()
...
test('invariant Dedalus custom commands share the generated program', () => {
  ...
  assert.ok(
    program.commands.some((command) => command.name() === 'completion'),
    'generated completion command is missing',
  )
})
  ...
  addDedalusCommands(program, {
    auth: () => ({
      ...
      status: async (_flags, value) => {
        offline = value
        return { source: 'oauth_session', offline: value, session: metadata }
      },
...
test('invariant human auth errors retain their stable code', async () => {
  ...
  addDedalusCommands(program, {
    auth: () => ({
      login: async () => {
        throw new AuthProviderError('access_denied', { stage: 'provider' })
      },
      status: async () => ({ source: 'none' }),
      logout: async () => ({ status: 'not_logged_in', revocationConfirmed: false }),
    }),
...
test('invariant provider errors preserve safe codes and retry policy', async () => {
  ...
  addDedalusCommands(program, {
    auth: () => ({
      login: async () => {
        throw new AuthProviderError('temporarily_unavailable', { stage: 'provider' })
      },
      status: async () => ({ source: 'none' }),
      logout: async () => ({ status: 'not_logged_in', revocationConfirmed: false }),
    }),
...
test('invariant local response validation remains in the CLI error namespace', async () => {
  ...
  addDedalusCommands(program, {
    auth: () => ({
      login: async () => {
        throw new AuthProviderError('invalid_token_response', { status: 200 })
      },
      status: async () => ({ source: 'none' }),
      logout: async () => ({ status: 'not_logged_in', revocationConfirmed: false }),
    }),
...
test('invariant provider error codes cannot inject terminal control characters', async () => {
  ...
  assert.equal(
    output,
    "oauth_error: Login could not be completed. Run 'dedalus auth login' again.\n",
  )
  process.exitCode = 0
})

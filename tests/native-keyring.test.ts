...
import { CredentialStorageError, defaultCredentialStore, keyringCredentialStore } from '../src/auth/credentials.js'
...
test('invariant native keyring failures retain their causes', async () => {
  ...
  const store = keyringCredentialStore(async () => ({
    getPassword: async () => { throw failure },
    setPassword: async () => { throw failure },
    deleteCredential: async () => { throw failure },
  }))
  for (const action of [() => store.read(), () => store.write(session), () => store.remove()]) {
    await assert.rejects(action, (error) =>
      error instanceof CredentialStorageError && error.code === 'storage_unavailable' && error.cause === failure)
  }
})
...
test('invariant releasing a lifecycle lock preserves an earlier operation failure', async (context) => {
  ...
  context.mock.method(lockfile, 'lock', async () => async () => { throw releaseFailure })
  await assert.rejects(defaultCredentialStore().withLifecycleLock(async () => { throw operationFailure }),
    (error) => error instanceof AggregateError &&
      error.errors.includes(operationFailure) &&
      error.errors.some((failure) => failure instanceof CredentialStorageError && failure.cause === releaseFailure))
})
// @custom end

...
test('invariant released ESM, CommonJS and executable versions follow the package', async () => {
  ...
  try {
    ...
    const run = (args: string[]) =>
      execFileSync(process.execPath, args, { cwd: directory, encoding: 'utf8' }).trim()
    run(['--import', 'tsx', 'scripts/prepare-build.ts'])
    ...
    assert.equal(run(['dist/esm/bin.js', '--version']), manifest.version)
    assert.equal(
      run([
        '--input-type=module',
        '-e',
        "import { getProgram } from './dist/esm/index.js'; console.log(getProgram().version())",
      ]),
      manifest.version,
    )
    assert.equal(
      run(['-e', "console.log(require('./dist/cjs/index.js').getProgram().version())"]),
      manifest.version,
    )
  } finally {

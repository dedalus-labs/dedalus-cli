...
const commandDefinition = (transport: 'http' | 'websocket') => ({
  resourcePath: ['probe'],
  commandPath: ['probe'],
  methodName: 'run',
  transport,
...
const programSource = (sdkSource: string, transport: 'http' | 'websocket') => `
...
const runProgram = (source: string, input: string, delayMs = 0) =>

...
import { awaitSSHSession, connectMachine } from '../dist/esm/custom/ssh.js'

const ready = {
  session_id: 'ss-test', status: 'ready',
  connection: {
    endpoint: 'fixture.example', port: 2222, ssh_username: 'root', user_certificate: 'fixture-cert',
...
test('invariant terminal errors and malformed polling responses stop without polling', async () => {
  for (const session of [
    ...
    { ...waiting, retry_after_ms: -1 }, { status: 'ready' },
  ]) {
    let polls = 0
    await assert.rejects(awaitSSHSession({
      createSSHSession: async () => session,
      getMachineSSHSession: async () => { polls++; return ready },
    }, 'dm-test', 'pub'))
...
test('invariant SSH uses server trust, propagates exit status and removes credentials', async () => {
  ...
  try {
    ...
    const executable = (source) => `#!${process.execPath}\n${source}\n`
    await writeFile(join(directory, 'ssh-keygen'), executable(`
    ...
    `), { mode: 0o755 })
    await writeFile(join(directory, 'ssh'), executable(`
      const fs = require('node:fs'); const args = process.argv.slice(2);
    ...
    await connectMachine({
      ...
      getMachineSSHSession: async () => { throw new Error('unexpected poll') },
    }, 'dm-test')
